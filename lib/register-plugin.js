...
module.exports = function (context) {
  ...
  return function (options) {
    ...
    if (!_.includes(validPluginHooks, hook)) {
      throw new Error(
        'hook must be one of the following: ' + validPluginHooks.join(', ')
      )
    }
    ...
    if (hook === 'static') {
      if (!_.isUndefined(context[name])) { throw new Error('plugin ' + name + ' is already defined.') }
